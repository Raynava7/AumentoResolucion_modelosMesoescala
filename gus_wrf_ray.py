...
def extract_hh_cubeRAY_InterpBiline(wrfdir,date_bgn,date_end,grid,wt_loc,wt_height):
	date_bgn = datetime.datetime.strptime(date_bgn,'%d-%m-%Y').date()
	date_end = datetime.datetime.strptime(date_end,'%d-%m-%Y').date()
	...
	for date in date_range:
		...
		#Place cubes in a list
		cube_list_ws[it] = U_hubh
		cube_list_wd[it] = V_hubh
		it = it + 1
	...
	#Create dataframes
	df_ws = iris.pandas.as_data_frame(cube_ws)
	df_ws = df_ws.rename(columns = {0:'U_m/s'})
	df_ws.index.name = 'Date'

	df_wd = iris.pandas.as_data_frame(cube_wd)
	df_wd = df_wd.rename(columns = {0:'V_m/s'})
	df_wd.index.name = 'Date'
...
def extract_hh_cubeRAY_InterpBiline_WS(wrfdir,date_bgn,date_end,grid,wt_loc,wt_height):
	date_bgn = datetime.datetime.strptime(date_bgn,'%d-%m-%Y').date()
	date_end = datetime.datetime.strptime(date_end,'%d-%m-%Y').date()
	...
	#Create dataframes
	df_ws = iris.pandas.as_data_frame(cube_ws)
	df_ws = df_ws.rename(columns = {0:'Vel_m/s'})
	df_ws.index.name = 'Date'

	df_wd = iris.pandas.as_data_frame(cube_wd)
	df_wd = df_wd.rename(columns = {0:'Dir'})
	df_wd.index.name = 'Date'
...
def extract_hh_cubeRAY_nearest(wrfdir,date_bgn,date_end,grid,wt_loc,wt_height):
	date_bgn = datetime.datetime.strptime(date_bgn,'%d-%m-%Y').date()
	date_end = datetime.datetime.strptime(date_end,'%d-%m-%Y').date()
	...
	for date in date_range:
		...
		#Horizontal interpolations
		U_site = U.interpolate(wt_loc, iris.analysis.Nearest())
		V_site = V.interpolate(wt_loc, iris.analysis.Nearest())
		U10_site = U10.interpolate(wt_loc, iris.analysis.Nearest())
		V10_site = V10.interpolate(wt_loc, iris.analysis.Nearest())
		PH_site = PH.interpolate(wt_loc, iris.analysis.Nearest())
		PHB_site = PHB.interpolate(wt_loc, iris.analysis.Nearest())
...
def extract_hh_cubeRAY_nearest_WS(wrfdir,date_bgn,date_end,grid,wt_loc,wt_height):
	date_bgn = datetime.datetime.strptime(date_bgn,'%d-%m-%Y').date()
	date_end = datetime.datetime.strptime(date_end,'%d-%m-%Y').date()
...
def extract_hh_cubeRAY_landu_index(wrfdir,date_bgn,date_end,grid,wt_loc,wt_height):
	date_bgn = datetime.datetime.strptime(date_bgn,'%d-%m-%Y').date()
	date_end = datetime.datetime.strptime(date_end,'%d-%m-%Y').date()
	...
	#Interpolation routine
	date_range = (date_bgn + datetime.timedelta(n) for n in range((date_end - date_bgn).days + 1)) #Generator
	cube_list_lu = [None] * len(range((date_end - date_bgn).days + 1))
	cube_list_wd = [None] * len(range((date_end - date_bgn).days + 1))
	it = 0
	for date in date_range:
		#print('\tReading files for: ' + date.strftime('%d') +'-'+ date.strftime('%m') +'-'+ date.strftime('%Y'))
		...
		files_ofday = (wrfdir+'/wrfout_d{}_{}-{}-{}_*'.format("%02d" % grid, date.strftime('%Y'), date.strftime('%m'), date.strftime('%d')))
		#Load files in a list of cubes (one cube per file)
		LU = iris.load(files_ofday,['LU_INDEX'])	# Indice de land use

		#Get in one cube all the time steps of the day (from a list of cubes)
		LU = concat_list(LU)        

		#Rename and order coordinates in cubes
		LU = fix_coordinates(LU)
        
		#Horizontal interpolations
		LU_site = LU.interpolate(wt_loc, iris.analysis.Nearest())
		...
		#U_hubh = vert_semilog_interp_1D(H_levels,U_site,U10_site,wt_height)
		#V_hubh = vert_semilog_interp_1D(H_levels,V_site,V10_site,wt_height)
		#LU_s = ws_ifunc(LU_site, LU_site, new_name='Land Use')
        
		#Place cubes in a list
		cube_list_lu[it] = LU_site
		it = it + 1

	#Create iris list, unifty time units and concatenate in time
	cube_list_lu = iris.cube.CubeList(cube_list_lu)
	iris.util.unify_time_units(cube_list_lu)
	cube_lu = cube_list_lu.concatenate(cube_list_lu)[0]
#
	##Create dataframes
	df_lu = iris.pandas.as_data_frame(cube_lu)
	df_lu = df_lu.rename(columns = {0:'Land_Use_Index'})
	df_lu.index.name = 'Date'
